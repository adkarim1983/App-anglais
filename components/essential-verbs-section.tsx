...
export function EssentialVerbsSection({ onVerbClick }: EssentialVerbsSectionProps) {
  ...
  return (
    <Card className="overflow-hidden bg-gradient-to-br from-amber-50 via-yellow-50 to-amber-50 dark:from-amber-950/30 dark:via-yellow-950/30 dark:to-amber-950/30 border-2 border-amber-200 dark:border-amber-800/50 backdrop-blur-xl">
      {/* Header cliquable */}
      <button
        onClick={() => setIsExpanded(!isExpanded)}
        className="w-full p-6 flex items-center justify-between hover:bg-amber-100/50 dark:hover:bg-amber-900/20 transition-all"
      >
      ...
      {isExpanded && (
        <div className="px-6 pb-6 space-y-6 animate-in slide-in-from-top-2 duration-300">
          ...
          <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6">
            {ESSENTIAL_VERBS_WITH_IDS.map((verb, index) => (
              <div
                key={verb.id}
